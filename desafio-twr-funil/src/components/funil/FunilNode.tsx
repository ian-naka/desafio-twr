...
export default function FunilNode({ id, data, selected }: { id: string; data: FunnelNodeData; selected?: boolean }) {
        ...
        setNodes((nds) =>
            nds.map((node) => {
                if (node.id === id) {
                    return {
                        ...node,
                        data: { ...node.data, title: tituloLimpo, views: acessosSeguros, conversions: conversoesSeguras },
                    };
                }
                return node;
            })
    ...
    return (
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            ...
            >
                <Handle type="target" position={Position.Left} className="!w-3 !h-3 bg-primary border-2 border-background z-20" />
